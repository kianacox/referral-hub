export const HOMEPAGE_INTRO = `This is my personal referral hub—a collection of referral links for brands I use regularly or have subscriptions with. The discounts are often better than the brands’ own promotions. If you’re interested in trying something new without paying full price, you can use my links below.`;

export const DISCOUNTS_INTRO =
  "These are the referral codes I have that offer discounts on brands I use. Use the filters below to narrow by category.";

export const LEARN_MORE_LABEL = "Learn more";
export const GET_OFFER_NOW = "Get offer";
export const COPY_CODE = "Copy code";
export const COPIED = "Copied!";
